...
def calc_iou(output, target, threshold):
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (points, labels, surface, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        near_iou = calc_iou(output[:, 1024:], labels[:, 1024:], threshold)
...
def evaluate(data_loader, model, device):
    ...
    for points, labels, surface in metric_logger.log_every(data_loader, 50, header):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # loss = criterion(outputs, labels) / log_sigma.exp() + log_sigma / 2
            # loss = loss.mean()
            
            # loss = criterion(output[:, :1024], labels[:, :1024]) + 
